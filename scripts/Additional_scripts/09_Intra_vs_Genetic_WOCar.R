...
setwd("/Users/calbouy/Documents/These_Guilia/Papier_3/")

################################################################################
###                       Data compilation at the species level.             ###
################################################################################
micro <- readRDS("Data/micro/micro_data3.RDS") 
micro <- micro$intra_variability_pop[Selc_sp,]

micro <- cbind(Family=Family,nb_fam=as.vector(table(Family)[Family]),micro)
colnames(micro) <- paste(colnames(micro),"micro",sep="_")

Genet_Data <- readRDS("Data/Traits/F_dataset_2.RDS")
...
F_data <- data.frame(Genet_Data,micro,row.names=rownames(micro),Sp_names=rownames(Genet_Data))
...
pruned.treeRabosky_sp <- drop.tip(Rabosky,setdiff(Rabosky$tip.label, rownames(micro))) 
...
micro_traits <- ratio[grep("_micro",as.character(ratio))]

modelpgls <- lapply(1:length(micro_traits),function(i){
  summary(pgls(F_data$Jt~F_data[,micro_traits[i]],data=comp.data)) 
})
names(modelpgls) <- micro_traits

pvalues <- sapply(1:length(modelpgls),function(i) modelpgls[[i]]$coefficients[2,4])
names(pvalues) <- micro_traits
...
### Selected traits 
vect <- c(8,2,13)
micro_traitspl <- micro_traits[vect]
mains_s <- mains[vect]
...
col.gp_micro <- cool[as.factor(F_data$Species)]
...
tiff("Figures/Figure_4/Extended_figure_10_woCAr.tiff",width=10,height=4,units="in",res=300,compression="lzw")
...
for(i in 1:3){
  plot(rnorm(1000),col="white",type="n",axes=F,xlab="",ylab=ylab_list[[i]],
       ...
       xlim=c(min(F_data[,micro_traitspl[i]])-0.1,max(F_data[,micro_traitspl[i]])+0.2),
  ...
  points(y=Genet_Data$Jt,x=F_data[,micro_traitspl[i]],bg=col.gp_macro,pch=21,col="black")
  abline(lm(Genet_Data$Jt~F_data[,micro_traitspl[i]]),col="grey31",lty=2)
  text(F_data[,micro_traitspl[i]],Genet_Data$Jt,labels=F_data$Sp_names,pos=pos_list[[i]],cex=0.7,offset=0.6)
...
#tiff("Figures/Figure_4/Figure_supMat_W0Car.tiff",width=18,height=9, units="cm",res=300,compression ="lzw")
jpeg("Figures/Figure_4/Extended_figure_11_woCAr.tiff",width=18,height=9, units="cm",res=300)
...
for(i in 1:10){
  ...
  abline(lm(Genet_Data$Jt~F_data[,micro_traitspl[i]]),col="grey31",lty=2,lwd=0.4)
  text(F_data[,micro_traitspl[i]],Genet_Data$Jt,labels=F_data$Sp_names,pos=pos_list[[i]],cex=0.4,offset=0.3)
